...
-- Safety Check for Required Modules
if not logic then error("CRITICAL: logic.lua failed to load!") end
if not ui then error("CRITICAL: ui.lua failed to load!") end
...
-- BOOT SEQUENCE: Clear state and force initial inventory scan
term.clear()
term.setCursorPos(1,1)
term.setTextColor(colors.yellow)
print("ALCH-OS v3.8: Initializing...")
        ...
        for _, step in ipairs(plan) do
            if not (startType == "awkward" and step.name == "minecraft:nether_wart") then
                ...
                if ing then 
                    ...
                    os.sleep(22) -- Standard brewing time
                end
            end
        end
        
        -- E. AUTO-STORAGE: Push directly to storage
        if not silent then print("Storing batch...") end
...
local function brewQuantityMenu(name)
    ...
    local items = {
        { text = "CONFIRM BREW", handler = function()
            term.clear()
            term.setCursorPos(1, 2)
            print("Quantity to Brew (Max " .. max .. "):")
...
function statusScreen()
    ...
    print("-- SYSTEM STATUS --")
    term.setTextColor(colors.white)
    print("Water:    " .. logic.getStock("_water") .. " / 20")
    print("Awkward:  " .. logic.getStock("_awkward") .. " / 15")
...
local function backgroundWorker()
    while true do
        ...
        if not isBrewing then
            ...
            -- C. AGGRESSIVE INVENTORY SORTER
            local keepers = {
                ["minecraft:potion"]=true, ["minecraft:glass_bottle"]=true, 
                ["minecraft:blaze_powder"]=true, ["minecraft:nether_wart"]=true, 
                ["minecraft:glass"]=true, ["minecraft:blaze_rod"]=true
            }
            -- Dynamically keep all recipe ingredients
